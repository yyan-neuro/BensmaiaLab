...
cc_sao_loaf = cell(numel(iNoiseRatios),numel(cNoiseRatios),Niter);
cc_dio_loaf = cell(numel(iNoiseRatios),numel(cNoiseRatios),Niter);
    ...
    for i_ind = 1:numel(iNoiseRatios)
        ...
        for c_ind = 1:numel(cNoiseRatios)
            ...
            for i = 1:numel(MatList_DS)
                ...
                [cc_sao_mat,cc_dio_mat] = simulationCC_DSData(graspMat_noisy,graspMat_ind);

                cc_sao_bigmat = [cc_sao_bigmat;cc_sao_mat];
                cc_dio_bigmat = [cc_dio_bigmat;cc_dio_mat];
            ...
            cc_sao_loaf{i_ind,c_ind,Niter_i} = cc_sao_bigmat;
            cc_dio_loaf{i_ind,c_ind,Niter_i} = cc_dio_bigmat;
